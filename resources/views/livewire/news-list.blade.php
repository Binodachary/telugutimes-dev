<div class="space-y-3">
    @forelse($newsItems as $newsItem)
        <div class="article-list-item p-2 bg-gray-100 shadow-md rounded">
            <a href="{{ route('article',$newsItem) }}" class="flex flex-col md:flex-row">
                <img src="{{ asset("/images/medium/{$newsItem->image}") }}" class="w-full rounded md:w-52 lg:h-32 h-52 object-cover object-center" alt="{{ $newsItem->title }}">
